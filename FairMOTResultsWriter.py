import os
import sys
sys.path.append(r"C:\Users\Bartek\Downloads\licznik_osob_i_pojazdow- 15.09.2025\DCNv2\build\lib.win-amd64-cpython-311")
import cv2
import logging
import argparse
...
# --- FairMOT imports ---
FAIRMOT_ROOT = r"C:\Users\Bartek\Downloads\licznik_osob_i_pojazdow- 15.09.2025\FairMOT"
SRC_PATH = os.path.join(FAIRMOT_ROOT, "src")
LIB_PATH = os.path.join(SRC_PATH, "lib")
sys.path.append(SRC_PATH)
sys.path.append(LIB_PATH)

from FairMOT.src.lib.models.model import create_model
from FairMOT.src.lib.models.model import load_model
...
class MOT16Writer:
    """Klasa do zapisywania wyników w formacie MOT16"""
...
def process_sequence(seq_name, tracker, writer):
    ...
    for frame_number, file_name in enumerate(tqdm(frame_files, desc=f"Processing {seq_name}")):
        ...
        # --- FairMOT processing ---
        online_targets = tracker.update(frame, frame)
        tracks = []
...
def main():
    parser = argparse.ArgumentParser(description='FairMOT Tracker for MOT16 JPG sequences')
    parser.add_argument('--cfg', type=str, default=r"C:\ścieżka\do\FairMOT\src\lib\cfg\mot\fairmot_dla34.yaml")
    parser.add_argument('--weights', type=str, default=r"C:\ścieżka\do\FairMOT\pretrained\fairmot_dla34.pth")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"

    logger.info("Ładowanie modelu FairMOT...")
    model = create_model(args.cfg, backbone_name="dla_34")
    model = load_model(model, args.weights, device=device)
    tracker = JDETracker(model=model, device=device)
    ...
    for seq_name in seq_list:
        process_sequence(seq_name, tracker, writer)
